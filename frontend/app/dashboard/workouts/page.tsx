...
export default async function WorkoutsPage() {
    ...
    const error = null; // pg throws on failure, so no "soft error" here
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {workouts.map((workout) => (
                        <Link key={workout.id} href={`/dashboard/workouts/${workout.id}`}>
                            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700 hover:shadow-md transition cursor-pointer h-full flex flex-col">
                                <div className="flex justify-between items-start mb-4">
                                    <h3 className="text-lg font-bold text-gray-900 dark:text-white truncate">
                                        {workout.title}
                                    </h3>
                                    <span
                                        className={`px-2 py-1 text-xs font-semibold rounded-full uppercase ${workout.source === "ai"
                                                ? "bg-purple-100 text-purple-700"
                                                : workout.source === "trainer"
